...
Public Class UploadCMR
    ...
    Protected Sub btnUpload_Click(sender As Object, e As EventArgs) Handles btnUpload.Click
        ...
        While (streamReader.Peek() > -1)
            ...
            If astrValues.Length <> 22 Then
                '''''changed 21 t0 22 to test Liber's new csv
            ...
            If RadioButtonList1.SelectedValue <> "Preview" Then
                If classtype = "E" Then
                    ...
                    Else
                        ...
                        Response.Write(IsCMRchange(term, reference, session, mdcid, status) & "  -myreturn <br>")
                        If IsCMRchange(term, reference, session, mdcid, status) = True Then  'did mdcid, status, or session change?
                            Response.Write("SAME<br>")
                        Else
                            Response.Write("CHANGED <br>")
                            changecount = changecount + 1
                            UpdateCMR(term, reference, session, startdate, enddate, mdcid, last, first, email, status, instAssign, Chair, chairoutlook)
    ...
    Private Sub UpdateCMR(ByVal term As String, ByVal reference As String, ByVal session As String, ByVal startdate As String, ByVal enddate As String, ByVal mdcid As String, ByVal last As String, ByVal first As String, ByVal email As String, ByVal status As String, ByVal InstAssign As String, ByVal chair As String, ByVal chairoutlook As String)
        ...
        If RadioButtonList1.SelectedValue = "Input" Then
            ...
            Catch
                ' failed to create a new record 
                Response.Write("<br>Fail CMRUpdate")
            Finally
                'Close the Connection
                If myConnection.State = ConnectionState.Open Then
                    myConnection.Close()
    ...
    Private Function IsCMRchange(ByVal myterm As String, ByVal myreference As String, ByVal mysession As String, ByVal mymdcid As String, ByVal mystatus As String) As Boolean

        Dim myreturn As Boolean = 0
        Dim mycount As Integer = 0
        Response.Write(mysession & "--myfilesession<br>")
        'Response.Write(mymdcid & "<br>")
        Dim strSQL As String = "Select Session, MDCID,status from CMR where term='" & myterm & "' and reference='" & myreference & "'"
        Dim myConnection As New SqlConnection(ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString)
        ' Response.Write(strSQL & "<br>")
        Dim myCommand As New SqlCommand(strSQL, myConnection)
        ...
        If dr.HasRows Then
            While dr.Read()
                ...
                If dr("Session") = mysession And dr("MDCID") = mymdcid And dr("status") = mystatus Then
                    myreturn = 1
                Else
                    myreturn = 0
                End If
